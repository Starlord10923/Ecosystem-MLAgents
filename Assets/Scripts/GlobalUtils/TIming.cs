...
namespace MEC
{
    public class Timing : MonoBehaviour
    {
        ...
        /// <summary>
        /// Returns true if the coroutine represented by <paramref name="handle"/> is still enqueued
        /// (i.e. has neither completed nor been killed).
        /// </summary>
        public static bool IsCoroutineRunning(CoroutineHandle handle)
        {
            // grab the right Timing instance
            Timing inst = GetInstance(handle.Key);
            if (inst == null) return false;

            // if we've never started or already killed it, it won't be in our map
            if (!inst._handleToIndex.ContainsKey(handle)) return false;

            // check that its slot hasn't been nullified
            ProcessIndex idx = inst._handleToIndex[handle];
            return !inst.CoindexIsNull(idx);
        }
    ...
    public struct CoroutineHandle : System.IEquatable<CoroutineHandle>
    {
        ...
        /// <summary>
        /// True if this handle is still enqueued (hasn't completed or been killed).
        /// </summary>
        public readonly bool IsRunning
        {
            get { return Timing.IsCoroutineRunning(this); }
        }
    }
    ...
    public static class MECExtensionMethods1
    {
        ...
        /// <summary>
        /// Returns true if this coroutine handle is still active in MEC.
        /// </summary>
        public static bool IsRunning(this CoroutineHandle handle)
        {
            return Timing.IsCoroutineRunning(handle);
        }
    }
}
